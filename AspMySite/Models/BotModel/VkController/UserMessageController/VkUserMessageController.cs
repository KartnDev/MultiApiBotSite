...
using AspMySite.Models.BotModel.BotComplexCommands.VkComplexCommands;
...
{
    public class VkUserMessageController : AbstractVkEventController
    {
       

        public VkUserMessageController(string VkToken) : base(VkToken)
        {
           
        }
        public override void ControlMessage(string message, List<AspMySite.Models.SQLModel.DbTableModels.VkCommands> commands, int chatID, List<ChatUser> chatUsers)
        {
            ...
            try
            {
                ...
                if (message.Contains(" "))
                {
                    ...
                    if (args[0] == "!ban" && args.Length >= 3)
                    {
                        ...
                        if (args.Length == 2)
                        {
                            vkCommands.Ban(id, int.Parse(args[2]));
                        }
                        else if (args.Length == 3)
                        {
                            vkCommands.Ban(id, int.Parse(args[2]), int.Parse(args[2]));
                        }
                        else if (args.Length == 4)
                        {
                            ...
                            vkCommands.Ban(AssosiatToID, int.Parse(args[2]), int.Parse(args[2]), args[4]);
                        }

                    }
                    if (args[0] == "!invite" && args.Length == 3)
                    {
                        int id = 0;
                        if (haveAssosiation && AssosiatToID != 0)
                        {
                        ...
                        vkMethods.Invite(id, int.Parse(args[2]));
                    }
